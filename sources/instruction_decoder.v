...
module instruction_decoder(
    ...
    // single-bit outputs: use packed 8-bit vectors (one bit per decoder)
    wire [7:0] cen_w;
    wire [7:0] rst_w;
    wire [7:0] oen_w;
    wire [7:0] inc_w;
    ...
    // two-bit buses: split into two bit-vectors (LSB and MSB)
    wire [7:0] a_mux_sel_w0; // bit 0 for each decoder
    wire [7:0] a_mux_sel_w1; // bit 1 for each decoder
    ...
    // ----------------------------------------------------
        ...
        a_mux_sel  = {a_mux_sel_w1[id], a_mux_sel_w0[id] };
        b_mux_sel  = {b_mux_sel_w1[id], b_mux_sel_w0[id] };
        push       = push_w[id];
        pop        = pop_w[id];
